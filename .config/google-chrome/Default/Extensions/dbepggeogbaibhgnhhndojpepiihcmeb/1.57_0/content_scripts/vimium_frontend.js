...
(function() {
  ...
  NormalMode = (function(superClass) {
    ...
    function NormalMode(options) {
      ...
      NormalMode.__super__.constructor.call(this, extend(options, {
        name: "normal",
        indicator: false,
  ...
  initializePreDomReady = function() {
    ...
    return chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
      ...
      if (!(request.handler && !request.name)) {
        if (isEnabledForUrl || ((ref = request.name) === "checkEnabledAfterURLChange" || ref === "runInTopFrame")) {
  ...
  Frame = {
    ...
    registerFrameId: function(arg1) {
      ...
      if (windowIsFocused() || !DomUtils.windowIsTooSmall()) {
        ...
        window.addEventListener("focus", focusHandler = function() {
          if (event.target === window) {
            return postRegisterFrame();
          }
        });
        return window.addEventListener("resize", resizeHandler = function() {
  ...
  extend(window, {
    ...
    focusInput: (function() {
      ...
      return function(count, mode) {
        var FocusSelector, element, elements, hint, hints, i, resultSet, selectedInputIndex, tuple, visibleInputs;
        if (mode == null) {
          mode = InsertMode;
        }
