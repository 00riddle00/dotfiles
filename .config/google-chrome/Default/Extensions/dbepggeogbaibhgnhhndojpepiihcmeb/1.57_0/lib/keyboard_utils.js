...
(function() {
  ...
  KeyboardUtils = {
    ...
    isEscape: function(event) {
      return (event.keyCode === this.keyCodes.ESC) || (event.ctrlKey && this.getKeyChar(event) === '[' && !event.metaKey && !event.altKey);
    },
    ...
    getKeyCharString: function(event) {
      ...
      switch (event.type) {
        case "keypress":
          ...
          break;
        case "keydown":
          keyChar = this.getKeyChar(event);
          if (1 < keyChar.length || (keyChar.length === 1 && (event.metaKey || event.ctrlKey || event.altKey))) {
            modifiers = [];
            if (event.shiftKey) {
