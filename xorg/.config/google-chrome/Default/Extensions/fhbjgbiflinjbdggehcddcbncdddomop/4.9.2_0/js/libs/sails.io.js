...
(function() {
  ...
  function SailsIOClient(io) {
    ...
    function request (url, callback) {
      ...
      xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState === (XMLHttpRequest.DONE || 4)) {
          ...
          xhr = null;
          (status !== 200) && console.log('error requesting ' + url);
          // finally do callback
          return callback(status !== 200 ? true : null, status);
        }
      };
    ...
    function SailsSocket (opts) {
      opts && consolog.noPrefix("🌏 sync service connectivity initiated");

      var self = this;
      !opts && (opts = {});

      // save initial configuration and pick from defaults
      Object.keys(SailsSocket.selfConfigurationDefaults).forEach(function (config) {
        self[config] = ((config in opts) ? opts : SailsSocket.selfConfigurationDefaults)[config];
      });
    ...
    SailsSocket.selfConfigurationDefaults = {
        ...
        reconnectionAttempts: 240
    };

    SailsSocket.sockConfigurableProperties = ['reconnectionDelay', 'reconnectionDelayMax', 'reconnectionAttempts']
    ...
    SailsSocket.prototype.configure = function (opts) {
        ...
        // Save only the new configurations provided
        Object.keys(opts).forEach(function (config) {
            (config in SailsSocket.selfConfigurationDefaults) && (self[config] = opts[config]);
        });

        if (!(self._raw && self._raw.io)) {
            consolog('Saved updated configuration, but did not update socket since it is not connected or available');
            return;
        }

        // configure live socket
        SailsSocket.sockConfigurableProperties.forEach(function (config) {
            (config in opts) && self._raw.io[config](opts[config]);
        });

        // handle backoff reset
        opts.reconnectionAttemptsReset && self._raw.io.backoff && self._raw.io.backoff.reset()
        consolog('Updated configuration of underlying socket');
    };

    /**
     * Reset reconnection attempt of a live socket
     */
    SailsSocket.prototype.reconnectionAttemptsReset = function () {
        return this._raw && this._raw.io && this._raw.io.backoff &&
            (this._raw.io.backoff.reset(), this._raw.io.reconnectionAttempts());
    };
    ...
    SailsSocket.prototype._prepare = function (callback) {
      ...
      // fire the connecting event
      try {
        self.__callbackOnConnecting && self.__callbackOnConnecting();
      }
      catch (e) {
      ...
      if (typeof window !== 'undefined') {
        return io.sails.useXHRForBrowserCORS ? request(xOriginCookieURL, callback) : jsonp({
    ...
    SailsSocket.prototype._connect = function (){
      ...
      self._prepare(function goAheadAndActuallyConnect() {
        ...
        self.on('connect', function socketConnected() {
          consolog.noPrefix("🌏 connected to sync service.");
        });

        self.on('disconnect', function() {
          self.connectionLostTimestamp = (new Date()).getTime();
          consolog.noPrefix("🌏 disconnected from sync service. possibly a network outage.");
        });

        self.on('reconnecting', function(numAttempts) {
          consolog.noPrefix("🌏 reconnecting to sync service. #(" + numAttempts + "/" + self.reconnectionAttempts +
            ")");
        });

        self.on('reconnect_scheduled', function (ms, attempt) {
          consolog.noPrefix("🌏 scheduled reconnection attempt #" + attempt + " after " + ms + "ms");
        });
        ...
        self.on('reconnect', function(transport, numAttempts) {
          ...
          consolog.noPrefix("🌏 reconnected to sync service after being offline for " + numSecsOffline + "seconds.");
        });

        self.on('reconnect_failed', function () {
          consolog.noPrefix("🌏 failed to reconnect to sync service after " + self.reconnectionAttempts + " attempts.");
        });

        self.on('connect_error', function (err) {
          consolog.noPrefix("🌏 error establishing connection with sync service.\n", err);
        });
        ...
        self.on('error', function failedToConnect(err) {
          ...
          // });
          consolog.noPrefix("🌏 error establishing connection with sync service. possibly network or firewall issue.\n",
            err);
        });
      });
    ...
    SailsSocket.prototype._reconnect = function () {
      var self = this,
        oldSocket;

      if (self.forceReconnecting) {
        throw new Error('already reconnecting socket. wait till connected event is fired');
      }
      // set the force reconnecting flag to true, this is unset later in the connect event
      self.forceReconnecting = true;

      if (self._raw) {
        consolog.noPrefix("🌏 force disconnecting from sync service.");
        // Bind the event to the raw underlying socket if possible.

        this._raw.disconnect();
        oldSocket = self._raw;
        delete self._raw; // @todo move it to disconnect event
      };
      ...
      self._prepare(function () {
        ...
        self._rebind(); // rebind all events from previous socket
        self.replay(); // Replay event bindings from the eager socket
      });
      return self;
    };
    ...
    SailsSocket.prototype.replay = function (){
      ...
      if ( !self.isConnected() ) {
        var whenRawSocketConnects = function () {
          console.log('socket replaying queued commands');
          // remove after connection / replay
          self._raw.off('connect', whenRawSocketConnects);
    ...
    SailsSocket.prototype.on = function (evName, fn){
      ...
      if (this._raw) {
        ...
        if (!this._events[evName]) { this._events[evName] = [fn]; }
        else { this._events[evName].push(fn); }
    ...
    SailsSocket.prototype.off = function (evName, fn){
      ...
      // Bind the event to the raw underlying socket if possible.
      if (this._raw) {
        (this._events[evName] && this._events[evName].indexOf(fn) > -1) &&
          this._events[evName].splice(this.events[evName].indexOf(fn), 1);
     ...
     */
    SailsSocket.prototype._rebind = function () {
      var self = this,
        eveName,
        i,
        ii;
      ...
      // Pass events and a reference to the request queue
        // off to the self._raw for consumption
        for (evName in self._events) {
          for (i = 0, ii = self._events[evName].length; i < ii; i++) {
            if (self._raw) {
              self._raw.on(evName, self._events[evName][i]);
            }
            else {
              !self.eventQueue[evName] && (self.eventQueue[evName] = []);
              self.eventQueue[evName].push(self._events[evName][i]);
            }
          }
        }
    };
    ...
    io.sails = {
      ...
      // use XHR instead of jsonp (needed in Chrome packaged apps)
      useXHRForBrowserCORS: true,
     ...
     * @return {Socket}
     */
    io.sails.connect = function(url, opts, events) {
      opts = opts || {};
      ...
      // attach construction events
      if (events) {
        for (evName in events) {
          // specially handle custom events
          if (evName === 'connecting') {
            socket.__callbackOnConnecting = events[evName];
            continue;
          }
  ...
  // Postman - Commented out on purpose
