...
(function() {
  var BackgroundCommands, DISABLED_ICON, ENABLED_ICON, Frames, HintCoordinator, PARTIAL_ICON, TabOperations, completers, completionHandlers, completionSources, currentVersion, cycleToFrame, fn, frameIdsForTab, getHelpDialogHtml, handleCompletions, handleFrameFocused, helpDialogHtmlForCommand, helpDialogHtmlForCommandGroup, i, icon, iconImageData, j, len, len1, mkRepeatCommand, moveTab, onURLChange, portHandlers, portsForTab, ref, ref1, removeTabsRelative, root, scale, selectSpecificTab, selectTab, sendRequestHandlers, showUpgradeMessage, toggleMuteTab,
  ...
  getHelpDialogHtml = function(arg) {
    var command, commandsToKey, customTitle, group, key, ref, ref1, replacementStrings, showCommandNames, showUnboundCommands;
    showUnboundCommands = arg.showUnboundCommands, showCommandNames = arg.showCommandNames, customTitle = arg.customTitle;
    commandsToKey = {};
    ...
    replacementStrings = {
      ...
      tip: showCommandNames ? "Tip: click command names to yank them to the clipboard." : "&nbsp;"
    };
    ref1 = Commands.commandGroups;
    for (group in ref1) {
      if (!hasProp.call(ref1, group)) continue;
      replacementStrings[group] = helpDialogHtmlForCommandGroup(group, commandsToKey, Commands.availableCommands, showUnboundCommands, showCommandNames);
    }
    return replacementStrings;
  };

  helpDialogHtmlForCommandGroup = function(group, commandsToKey, availableCommands, showUnboundCommands, showCommandNames) {
    var bindings, command, description, html, i, isAdvanced, key, keys, len, ref;
    html = [];
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      bindings = ((function() {
        ...
        for (j = 0, len1 = keys.length; j < len1; j++) {
          key = keys[j];
          results.push("<span class='vimiumHelpDialogKey'>" + (Utils.escapeHtml(key)) + "</span>");
        }
        return results;
      })()).join(", ");
  ...
  BackgroundCommands = {
    createTab: mkRepeatCommand(function(request, callback) {
      if (request.url == null) {
        request.url = (function() {
          ...
          if (url === "pages/blank.html") {
            if (request.tab.incognito) {
              return "chrome://newtab";
            } else {
              return chrome.runtime.getURL(newTabUrl);
            }
          } else {
            return url;
          }
      ...
      return TabOperations.openUrlInNewTab(request, function(tab) {
  ...
  sendRequestHandlers = {
    ...
    log: function(arg, sender) {
      var frameId, message;
      frameId = arg.frameId, message = arg.message;
  ...
  (showUpgradeMessage = function() {
    var notification, notificationId, ref2;
